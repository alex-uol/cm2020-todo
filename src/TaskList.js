...
function TaskList({
  ...
  const sortedTasks = () => {
    switch (sortOption) {
      case "byCategory":
        return renderTasksByGroup(
          sortTasksByCategory(filteredTasks),
          categories,
          "category",
        );
      case "byTags":
  ...
  const renderTasksByGroup = (groupedTasks, groupArray, groupType) => {
    return Array.from(groupedTasks.entries()).map(([groupId, tasks]) => {
      ...
      return (
        <div key={groupId}>
          {groupType === "category" && group && (
            <div>
              <strong>{`Category: ${group.title}`}</strong>
            </div>
          )}
          {groupType === "tag" && group && (
            <div>
              <strong>{`Tag: ${group.name}`}</strong>
            </div>
          )}
          {renderTasks(tasks)}
        </div>
    ...
    <div className="task-list">
      ...
      <div className="filter-buttons">
        ...
        {filterStatus !== Status.completed && (
          <button
            onClick={() => setFilterStatus(Status.completed /*'completed'*/)}
          >
            Show Completed
          </button>
        )}
        {filterStatus !== Status.active && (
          <button
            onClick={() => setFilterStatus(Status.active /*'uncompleted'*/)}
          >
            Show Uncompleted
          </button>
        )}
        {filterStatus !== Status.trashed && (
          <button onClick={() => setFilterStatus(Status.trashed /*'trashed'*/)}>
            Show Trashed
          </button>
        )}
      </div>
      ...
      <div className="sorting-buttons">
        <button
          className={sortOption == "unsorted" ? "active" : ""}
          onClick={() => setSortOption("unsorted")}
        >
          Unsorted
        </button>
        <button
          className={sortOption == "byCategory" ? "active" : ""}
          onClick={() => setSortOption("byCategory")}
        >
          Sort by Category
        </button>
        <button
          className={sortOption == "byTags" ? "active" : ""}
          onClick={() => setSortOption("byTags")}
        >
          Sort by Tags
        </button>
        <button
          className={sortOption == "byPriority" ? "active" : ""}
          onClick={() => setSortOption("byPriority")}
        >
          Sort by Priority
        </button>
        <button
          className={sortOption == "bySubTasks" ? "active" : ""}
          onClick={() => setSortOption("bySubTasks")}
        >
          Sort Tasks with Sub-Tasks
        </button>
      </div>
      <div>{sortedTasks()}</div>
    </div>
